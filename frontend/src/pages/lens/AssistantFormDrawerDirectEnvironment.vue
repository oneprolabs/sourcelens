      ...
      <template v-if="form.mode === 'direct'">
        <fieldset class="datasource-picker space-y-4 rounded-xl border border-line bg-surface p-4">
          ...
          <div v-if="datasourceOptions.length" class="datasource-select">
            <button type="button" class="datasource-select-trigger" :aria-expanded="datasourceMenuOpen" @click="datasourceMenuOpen = !datasourceMenuOpen">
              <span class="min-w-0 flex-1 truncate">{{ selectedDatasourceSummary }}</span>
              <span class="text-xs text-ink-400">⌄</span>
            </button>
            <div v-if="datasourceMenuOpen" class="datasource-select-menu">
              <input v-model="datasourceSearch" class="form-input mb-2" type="search" :placeholder="t('lensAdmin.datasourceSelection.search')" />
              <label v-for="source in filteredDatasourceOptions" :key="source.uuid" class="datasource-option">
              ...
              <div v-for="source in filteredDatasourceOptions" :key="`${source.uuid}-items`">
                <div v-if="hasSource(source) && source.items?.length > 1" class="ml-6 border-l border-line pl-2">
                  <label v-for="item in source.items" :key="item.uuid" class="datasource-item-option">
              ...
              <p v-if="!filteredDatasourceOptions.length" class="p-3 text-center text-xs text-ink-500">{{ t('lensAdmin.datasourceSelection.empty') }}</p>
            </div>
          </div>
...
const selectedDatasourceCount = computed(() => new Set((props.form.datasource_bindings || []).map((binding) => binding.datasource_uuid)).size)
const selectedDatasourceSummary = computed(() => {
  const selected = props.datasourceOptions.filter((source) => hasSource(source))
  if (!selected.length) return t('lensAdmin.datasourceSelection.placeholder')
  const names = selected.slice(0, 2).map((source) => source.name).join(', ')
...
const filteredDatasourceOptions = computed(() => {
  ...
  return props.datasourceOptions.filter((source) => !query || source.name.toLowerCase().includes(query))
})
...
.datasource-select { position: relative; }
.datasource-select-trigger { @apply flex w-full items-center gap-2 rounded-lg border border-line bg-surface-sunken px-3 py-2.5 text-left text-sm text-ink-700; }
.datasource-select-menu { @apply absolute z-20 mt-1 max-h-80 w-full overflow-y-auto rounded-lg border border-line bg-surface p-2 shadow-lg; }
.datasource-option { @apply flex cursor-pointer items-center gap-2 rounded-md px-2 py-2 text-sm hover:bg-surface-sunken; }
.datasource-item-option { @apply flex cursor-pointer items-center gap-2 py-1.5 text-xs text-ink-600; }
</style>
